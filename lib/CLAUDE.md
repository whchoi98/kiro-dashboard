...
| `refresh.tsx` | Global manual-refresh context — Header bumps `nonce`, every client fetch effect depends on it (pinned by tests/structure/refresh-wiring.test.ts); router.refresh() alone cannot re-run client effects |
| `report-html.ts` | Pure HTML document builder for exec report — `buildExecReportHtml(input)` generates a standalone HTML document (tables-only layout for Word compatibility); `wrapForWord(html)` wraps HTML with BOM + Word namespaces; `escapeHtml(s)` escapes all five HTML special characters; all values go through escaping before render; covered by `tests/lib/report-html.test.ts` |
...
| `export-report.ts` | Client exporters — `exportMarkdown` for AI answers, `exportPdf` via `html2canvas-pro` + `jspdf`, and `downloadBlob` for generic Blob → file downloads (used by exec report HTML/DOC export); Markdown/PDF labels follow a `locale` param (default `'ko'`); all use dynamic imports |
