...
| `export-report.ts` | Client exporters — `exportMarkdown` for AI answers (static), `exportPdf` via `html2canvas-pro` + `jspdf` (dynamic imports), and `downloadBlob` for generic Blob → file downloads (used by exec report HTML/DOC export); Markdown/PDF labels follow a `locale` param (default `'ko'`) |
