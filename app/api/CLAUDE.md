...
- The `idc-users` endpoint is a thin wrapper over `lib/idc-users.ts`, which walks IdentityStore directly (own client, not lib/identity.ts), joins Athena activity stats and the S3 first-seen ledger, and masks server-side
